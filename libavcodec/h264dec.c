...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size)
{
    ...
    for (i = 0; i < h->pkt.nb_nals; i++) { //read the NALs in this frame orderly
        H2645NAL *nal = &h->pkt.nals[i];
        ...
        h->nal_ref_idc   = nal->ref_idc;//indicate the NAL unit's reference status. It specifies whether the content of the NAL unit is used to reference other pictures (i.e., if it should be stored for future reference)
